...
#read data 
otu_tb16_bacteria_plus_chlor <- read.table(file="/home/laura/Documents/TFM/home/data/MALASPINA/16S_otu_table_no_hyphen.txt", head=TRUE, fill=TRUE)

#table dimensions and format before setting column names
#dim(otu_tb16_bacteria_plus_chlor) # 1315   125
#otu_tb16_bacteria_plus_chlor[1:5,1:5]

#row names = OTU name (option A)
row.names(otu_tb16_bacteria_plus_chlor)<-otu_tb16_bacteria_plus_chlor[,1]

#row names = row number (option B)
#rownames(otu_tb18) <- 1:nrow(otu_tb18)

otu_tb16_bacteria_plus_chlor<-otu_tb16_bacteria_plus_chlor[,-1]
otu_tb16_bacteria_plus_chlor[is.na(otu_tb16_bacteria_plus_chlor)]<-0

dim(otu_tb16_bacteria_plus_chlor) #  1315  124
otu_tb16_bacteria_plus_chlor[1:5,1:5]

#remove OTUs corresponding to chloroplasts
otu_tb16_5000<-otu_tb16_bacteria_plus_chlor[!rownames(otu_tb16_bacteria_plus_chlor) %in% c("3", "4"), ]
...
#the samples MD2304, MD2332, MD2695 and MD2926 have less than 5000 OTUs 

#remove samples with less than 5000 OTUs in the MP_18S_ss5000 dataset (so that we can compare the relative abundance of 16S and 18S OTUs considering the same samples)
...
amplicons_per_sample[which(colSums(otu_tb16_min5000_v2)>156000)]
```
...
There are 177 OTUs that don't show any occurrence in the normalized data. Let's remove them from the table and take a look at its final dimensions:
...
#The final dimensions of the normalized table are 119 1138.
#177+1318 = 1315
```

Datasets summary:

otu_tb16_min5000_v2_t --> 119 1315

otu_tb16_t_ss5000_no_cero --> 119 1138
...
Most of the samples get values between 0.425 and 0.6, meaning that the numerical composition of different OTUs in a sample is relatively similar:
...
The OTU_1 with 238811 reads, is the most abundant in the overall dataset:
...
Preston's lognormal model fit into species frequencies groups (we're missing ~67 species):
...
Preston's lognormal model fit without pooling data into groups (we're missing ~24 species):
...
The index Bray-Curtis allows us to quantify the differences between samples according to the composition and relative abundance of their OTUs. In our dataset, most of the samples pairs take dissimilarity values between 0.25 and 0.5, meaning that their composition is substantially different.
...
The only relatively evident clusters we can distinguish in the dendogram stand out in the very left side of the plot. The central cluster ranging from samples MD256 to MD2156 could also be informative. TBD: assign Longhurst provinces information to each station.
...
We can identify a prominent group in the middle right side of the NMDS plot and a few isolated samples in the right edge of the plot. The stress parameter takes a value below 0.3 (0.1503428), meaning that the plot is valid. 
...
plot(geo_distances_MP_16S_ss5000, otu_tb16_t_ss5000_no_cero.bray, pch=19, cex=0.4, xlab="Geopgraphic distances", ylab="Bray-Curtis dissimilarities")
```

## 4.1) Mantel correlograms

Mantel statistic is so low, meaning that the correlation between samples dissimilarity and geographical distances is weak; besides, it is not significant, so we shouldn't consider the test meaningful.
...
Regionally abundant OTUs (frequence over 0.1%):
...
tb16_ss5000_abundant[order(-tb16_ss5000_abundant[,1]),]

dim(otu_tb16_ss5000_rabund_percoccur[otu_tb16_ss5000_rabund_percoccur$mean_rabund > 0.001,])  
dim(tb16_ss5000_abundant)

#there are 157 regionally abundant OTUs.
(82/1138)*100 # = 7.29% of the OTUs are regionally abundant
...
Cosmopolitan OTUs (frequence over 0.1% and occurence in more than 80% of samples):

```{r select_cosmopolitan, echo=4}
...
```{r rare_OTUs, echo=T}
dim(otu_tb16_ss5000_rabund_percoccur[otu_tb16_ss5000_rabund_percoccur$mean_rabund < 0.00001 & otu_tb16_ss5000_rabund_percoccur$mean_rabund >0,]) # 325 OTUs
...
(325/1138)*100 # = 28.56% of the OTUs are regionally rare
```
